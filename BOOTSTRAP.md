...
8. **Seat harness, provider, auth route and model — a round of its own, run after the crew is chosen, walked over the seats question 7 ACTUALLY took.** Never one blanket question. A combined harness/provider/model question fails the same two ways the old provider+model question failed: its options cannot reflect a mixed crew, and its model options come from the interviewer's memory instead of the selected harness's account. Walk each taken seat by name, one at a time. Every seat gets its own explicit answer: "same as the previous seat" is an answer the principal gives, not a default you assume.
   ...
   Ask these as separate rounds for each seat. **Round 1 is only the harness.** The `AskUserQuestion` call has one question, it literally asks `Which harness?`, and its option labels are exactly `pi`, `claude-code`, `codex`. Do not include provider, auth route, or model text in that question or in those option labels.

   After the harness answer lands, ask the next separate round for that same seat. **Round 2 is only the provider**, narrowed by the chosen harness: `pi` uses today's live `pi --list-models` providers; `claude-code` offers only `anthropic`; `codex` offers only `openai-codex` or `openai`. Do not include auth routes or model ids in provider labels.

   After the provider answer lands, ask the next separate round. **Round 3 is only the auth route**, narrowed by the chosen harness/provider: `pi` offers `oauth`, `api_key`, `env`; `claude-code` offers `oauth` as the default Anthropic-subscription route and `api_key` only as the metered/non-default route; `codex` offers `oauth` or `api_key`. The option labels are exactly route values such as `oauth` and `api_key`, never `auth_json + ...`, `env_var + ...`, or any label that also names a model.

   After the auth answer lands, ask the final separate round. **Round 4 is only the model**, using model ids from the selected harness/account's live listing or binding probe. A model question that includes every harness's options is a failed Q8, not a helpful shortcut. A model option label is just the model id for the selected provider/harness — never a row combining harness, provider, auth, and model. Do not offer an `Other`, `custom`, `notes`, `same as previous`, or free-text label for ordinary choices; free prose is only the fallback when the tool is absent or when the principal explicitly chooses a correction path you named.

   Known-good `AskUserQuestion` shapes for the standard mixed scratch roster are intentionally boring: Q7 labels `2 workers`, `1 reviewer`, `Take verifier`, `Take designer`; each Q8 harness round labels `pi`, `claude-code`, `codex`; provider/auth/model labels follow only after the prior answer. The epic's falsifier roster is mixed — worker on Claude Code, reviewer on Codex, verifier on Pi — so the interview must not assume one harness for the fleet.
