...
3. **Verify the harness login before the binding probe**:
   - `pi`: `PI_CODING_AGENT_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" pi auth check --provider <provider> --json` (or the current Pi equivalent when the CLI names it differently).
