...
7. **Seats — the namespace once, then the roster walked one seat at a time.** Do not offer a fleet shape and ask whether they want it. A roster proposed whole is accepted whole, and the seat that goes missing is the one nobody was asked about: today that is reliably the designer, because its absence looks like a smaller install rather than a gap. If the installer is Claude Code and has `AskUserQuestion`, run this roster walk through the tool: option lists per seat for take/carry or decline, provider, and pinned model; free prose is only the fallback when the tool is absent. Field precedent: a real five-seat migration fit in two `AskUserQuestion` rounds.
   ...
   **For each seat taken, collect four facts — they are the roster's columns, and `seats/seats.json` records exactly these:**
   ...
   - **Provider**: whose model, and on whose account. Two routes are measured (2026-08-30) and anything else `pi` supports is legitimate:
     - `openai-codex` — a ChatGPT subscription, via OAuth: launch `PI_CODING_AGENT_DIR=... pi`, type `/login` inside the interactive REPL, complete the browser flow, then type `/exit`; that writes the seat's `auth.json` for that account. Probed working headlessly on a real subscription.
...
- Write `wheelhouse/ISA.md` using `INTENT.md` as the grammar spec: the goal from Q3, empty claims, empty decisions, and any anti-claims stated. **Do not invent claims.** An ISA with fabricated claims is worse than an empty one.
