...
`bun` is still unconditional because it runs the adapter, verifier/walker dispatchers, prune tool and floor for every local seat harness. `claude` is unconditional for the commander: the commander seat is a Claude Code session in the install root, and it is the one seat every install has. `pi`, `claude`, and `codex` as seat harness binaries are checked again after question 8, once the roster says which harnesses any non-external seats actually use; a no-worker install should not fail because a seat harness it never chose is absent.
...
set -- \
  ...
  "bun:runs the seat adapter, reviewer/verifier dispatchers, prune tool and floor (seats/adapter.ts, verify.ts, walk.ts, prune.ts, floor.ts) -- without it seats cannot be spawned, dispatched, walked, or pruned:brew install oven-sh/bun/bun   # or see https://bun.sh" \
...
1. **Product repos, and therefore the shape of this install.** "I see \<what you found\>. Are these the repos the fleet will change, and are they separate git repos from this root?"
   ...
   - **Umbrella root.** This directory is a container; the product lives in one or more separate git repos beneath it. The wheelhouse installs at the container root, the graph spans the repos below it, and workers create worktrees at the container root (for example `<root>/.wheelhouse-worktrees/<bead-id>`), never inside a product repo. That keeps the graph reachable from inside the worktree, which `contracts/GRAPH.md` explains (a worktree's `.git` is a file the workspace walk passes through; a nested repo's `.git` is a directory that stops it).
   - **Single repo.** This directory IS the product — one repo, no container. This is the ordinary case and it is fully supported: the wheelhouse installs at the product root, `CLAUDE.md` and `wheelhouse/` are committed alongside the code, and a worker's worktree is inside that root at `<root>/.wheelhouse-worktrees/<bead-id>`. That is the path `seats/adapter.ts` dispatches to, `seats/prune.ts` scans, and `seats/seat-env.sh`'s root trust grant covers. Because it is machine-local fleet runtime, exclude `.wheelhouse-worktrees/` in `.git/info/exclude`, not in the product's committed `.gitignore`.
...
7. **Seats — the namespace once, then the roster walked one seat at a time.** Do not offer a fleet shape and ask whether they want it. A roster proposed whole is accepted whole, and the seat that goes missing is the one nobody was asked about: today that is reliably the designer, because its absence looks like a smaller install rather than a gap. If the installer is Claude Code and has `AskUserQuestion`, run this roster walk through the tool: option lists per seat for take/carry or decline; free prose is only the fallback when the tool is absent. This question decides WHO is on the roster and nothing else — each seat's provider, auth route and model are question 8, a separate round walked over the seats this one actually takes, and asking them here as one combined question is the measured defect question 8 opens with.
   ...
   **The namespace comes first, because everything you are about to write sits under it.** Derive the default deterministically from the exact project directory name, lowercased and made filesystem-safe as one path segment (for example, collapse or replace characters the filesystem should not carry in a seat-root name; do not shorten or rebrand it). That default is the rule, not a suggestion. Correct it only for an actual collision on this machine or because the principal explicitly chooses a different namespace. Say what it decides: this project's seat root is `$HOME/.pi-seats-<namespace>`, the directory every seat's account lands under and the value `seats/seat-env.sh` is invoked with. Ask them to confirm or correct the derived string, not to design the scheme. Say why in one sentence, because a principal who does not know the reason will shorten it back out later: a machine can host more than one wheelhouse, and per-project seat roots are what keep two fleets' accounts apart on disk — same directory would mean same `auth.json` would mean one login serving two fleets, which is the collision `wheelhouse/fleet/SEATS.md`'s seat-accounting section forbids. That is the namespace's ONLY job now: seat NAMES are `seats/seats.json` keys, addressed per-project by the adapter, and carry no prefix — `worker-1`, not `<namespace>-worker-1`.
   ...
   The commander is not a question. It is the principal's own session in the directory whose `CLAUDE.md` you are about to write, so it exists whether or not anyone names it; it goes into `seats.json` as the `commander` entry, marked `"external": true`, because it runs on its own harness and the adapter manages nothing about it. Say that, and ask whether that commander session is launched directly or under a wrapper/status-line layer; if it is wrapped, point them at `seats/README.md`'s `WHEELHOUSE_HERALD_IDLE_RE` measurement recipe before choosing an idle regex. Then walk the rest individually — each answer is yes-with-a-count or no. Keep each prompt about its own role only: do not mention another role name inside the worker, reviewer, verifier, or designer question, because a tool-driven fixture or a human scanning quickly can classify the question by the wrong role. With `AskUserQuestion`, use labels shaped like the records you will write: `2 workers`, `1 reviewer`, `Take verifier`, `Take designer`, and matching decline labels.
...
8. **Seat harness, provider, auth route and model — a round of its own, run after the crew is chosen, walked over the seats question 7 ACTUALLY took.** Never one blanket question. A combined harness/provider/model question fails the same two ways the old provider+model question failed: its options cannot reflect a mixed crew, and its model options come from the interviewer's memory instead of the selected harness's account. Walk each taken seat by name, one at a time. Every seat gets its own explicit answer: "same as the previous seat" is an answer the principal gives, not a default you assume.

   Ask Q8 as four separate **choice rounds** after Q7 has chosen the roster. A round may contain one question per taken seat in a single `AskUserQuestion` call, but a round never mixes question kinds. This keeps the turn count bounded while preserving the narrowing order. The harness round is unmistakably separate: every harness question has its own header shaped exactly `Question 8 — <seat> harness`, its option labels are one label per option (`pi`, `claude-code`, `codex`), and the question text says explicitly that provider, auth route, and model are asked AFTER the harness answer and are never combined with it.

   **Round 1 is only harness choices for all taken seats.** Each question literally asks `Which harness?`, and its option labels are exactly `pi`, `claude-code`, `codex`. Do not include provider, auth route, or model text in that question or in those option labels.

   **Round 2 is only provider choices for all taken seats**, asked after the harness answers land and narrowed by each seat's chosen harness: `pi` uses today's live `pi --list-models` providers; `claude-code` offers only `anthropic`; `codex` offers only `openai-codex` or `openai`. Do not include auth routes or model ids in provider labels.

   For the `pi` harness:

   - **Provider — with a role-sensible proposal to correct.** Three routes are measured (2026-08-30 through 2026-09-02) and anything else `pi` supports is legitimate: `openai-codex` (a ChatGPT subscription, via OAuth), `openai` (an OpenAI API key, metered), `anthropic` (an Anthropic API key, metered), and OpenRouter when the live listing prints it for the seat. The proposals, each shaped by the role rather than by a scheme: a worker → the subscription route if the principal holds one, because implementation is the volume work and flat-rate absorbs it; the reviewer → an API-key provider on an identity distinct from every worker's, with the default offer being a mid-tier or cheap strong gate such as an Opus-class model or a GLM-class model through OpenRouter rather than the most expensive frontier option; the verifier → lead with the cheapest listed tool-capable provider/model row you can defend, commonly a GLM-class OpenRouter row when one prints, before offering Anthropic or other deeper metered rows, because the empty context is what matters, not the model's depth — the walker follows instructions on a surface and gates claims, not code review; the designer → plain `openai` by API key when the principal has that key, else the subscription route if it has headroom, else another API-key provider the principal names. If the installer asks provider as an option list, the designer/API-key path must be a visible `openai` option, not hidden behind "otherwise API-key provider" while the labels show only `openai-codex`, `anthropic`, and `google`; that exact omission forced a free-text answer in a live interview. Proposals, not a scheme to defend — the principal corrects, per seat. One fact is load-bearing at exactly this moment: **Anthropic subscription OAuth is NOT a route for a pi seat** — the API rejects third-party subscription auth outright (measured twice, 400 "third-party apps draw from extra usage"), so an Anthropic `pi` seat is an `api_key` or `env` seat. The `claude-code` harness route below is the Anthropic-subscription path for seats that should use Claude Code instead of Pi.

   **Round 3 is only auth-route choices for all taken seats**, asked after the provider answers land and narrowed by each seat's chosen harness/provider: `pi` offers `oauth`, `api_key`, `env`; `claude-code` offers `oauth` as the default Anthropic-subscription route and `api_key` only as the metered/non-default route; `codex` offers `oauth` or `api_key`. The option labels are exactly route values such as `oauth` and `api_key`, never `auth_json + ...`, `env_var + ...`, or any label that also names a model.
   ...
   **Round 4 is only model choices for all taken seats**, asked after the auth answers land and using model ids from each seat's selected harness/account listing or binding probe. A model question that includes every harness's options is a failed Q8, not a helpful shortcut. A model option label is just the model id for that seat's selected provider/harness — never a row combining harness, provider, auth, and model. Do not offer an `Other`, `custom`, `notes`, `same as previous`, or free-text label for ordinary choices; free prose is only the fallback when the tool is absent or when the principal explicitly chooses a correction path you named.

   For the `pi` harness:

   - **Model — pinned from a LIVE listing, taken at the moment of asking.** Run, now, on this machine: `pi --list-models | awk '$1=="<provider>"'` — and offer only ids that command just printed. The filter is the awk, because the command's optional search argument is a fuzzy match over whole rows, not a provider filter (measured: searching `openai-codex` returned `openai` and `openrouter` rows too). A model the runtime does not list for that provider is a typo the seat discovers at its first dispatch. Ids collide across providers (`gpt-5.4` exists under both `openai-codex` and plain `openai`, with different limits), which is one more reason provider and model are always passed together. Recommend one id from what printed and say why — for the reviewer, lead with a mid-tier or cheap strong default such as an Opus-class or GLM-class row, because reviewer seats run on every bead and every rework; for the verifier, lead with the cheapest listed id that still handles tool use, and make the first option the cheapest tool-capable row you are recommending rather than an Anthropic/frontier row unless Anthropic is genuinely the cheapest listed tool-capable option; the explicit expensive option is the provider's frontier row, called out as expensive with this worked example beside it: in one measured 2026-09-02 shadow day, the frontier gate cost about $120 total, about $5.70 per review round, while GLM 5.3 flash via OpenRouter cost $0.79 total, about $0.04 per round, and a flat subscription had $0 marginal cost. For workers, recommend a high-context id with strong tool use. The recommendation is a row the listing just produced, never a name this file remembers. Two gotchas are load-bearing (bead l0e): the model must be pinned in the roster, because a bare `--provider` silently falls back to the DEFAULT provider rather than erroring; and a ChatGPT-subscription account accepts only ids from the `openai-codex` listing — never propose an id from the plain `openai` (API-key) listing to a subscription seat, however similar it looks.

   Known-good `AskUserQuestion` shapes for the standard mixed scratch roster are intentionally boring: Q7 labels `2 workers`, `1 reviewer`, `Take verifier`, `Take designer`; the Q8 harness round labels `pi`, `claude-code`, `codex` for each taken seat; provider/auth/model labels follow only after the prior round's answers. The epic's falsifier roster is mixed — worker on Claude Code, reviewer on Codex, verifier on Pi — so the interview must not assume one harness for the fleet.

   **Anthropic subscription OAuth is not a route THROUGH PI** (measured twice, 400 "third-party apps draw from extra usage"). Through the `claude-code` harness it IS the route — measured 2026-09-15: `claude auth status` reports `authMethod claude.ai`, `subscriptionType max`, and every session init reports `apiKeySource none` — and that is why the harness column exists. A worker that should run on an Anthropic subscription is a `claude-code` seat with `account.authRoute` `oauth`; an `anthropic` seat on `pi` is still an `api_key` or `env` seat, never subscription OAuth.

   **Model — pinned from the selected harness's own live source, taken at the moment of asking.** For `pi`, run `PI_CODING_AGENT_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" pi --list-models | awk '$1=="<provider>"'` and offer only ids it prints for that provider. For `claude-code`, there is no list command in `claude --help`; offer the documented aliases as option labels — `fable`, `opus`, `sonnet`, `haiku` — but do not write the alias as the pin; record the full id the binding probe's `system/init` reports as the pinned model (for example, `sonnet` resolves to `claude-sonnet-5`). For `codex`, use the driver/app-server model list (`printf '{"jsonrpc":"2.0","id":1,"method":"model/list","params":{}}\n' | CODEX_HOME="$HOME/.pi-seats-<namespace>/<seat-name>" codex app-server`) and never a remembered catalog. If `model/list` returns no rows in this scratch harness or for this account, STOP the model round for that seat and ask for login/provisioning; do not offer remembered `gpt-*` ids, do not call them provisional, and do not substitute whatever model names you know. A model the selected harness/account cannot list or resolve is a typo the seat discovers at first dispatch.
     ...
     **The listing is credential-resolved, and so is the catalog behind it** (measured on pi 0.84.4, 2026-09-01): a provider prints only if its credential resolves for the process running the command, and the catalog refresh (`pi update --models`) fetches only for providers whose credentials resolve — a fresh agent directory's refresh fetched the env-key providers and skipped the OAuth one. So there is no pre-login listing for an OAuth provider on a machine that has never held that login, and a provider absent from YOUR listing may simply be one you are not logged into, not one pi lacks. When the filter prints nothing for a chosen provider, two honest paths, offered in this order:
     ...
     2. **Defer the listing only with a recorded provisional pin.** Record the role-sensible proposal as provisional, and make step 4's listing/resolution check mandatory before the probe counts.
...
1. **Provision the directory**: `seats/seat-env.sh <namespace> <seat-name>` — run once, safe to re-run. It creates `$HOME/.pi-seats-<namespace>/<seat-name>/` and writes `$HOME/.pi-seats-<namespace>/.project` as the backlink to this canonical project root (or STOPS if that namespace root already names a different project). The path name is historical and shared by every harness. For a `pi` seat, provisioning also pre-grants Pi trust for the project root and prints `PI_CODING_AGENT_DIR=... pi` login help when needed. For a `claude-code` seat, it must not require the `pi` binary and must not write `trust.json`; it prints `CLAUDE_CONFIG_DIR=... claude auth login --claudeai`. For a `codex` seat, it must not require `pi` and must not write `trust.json`; it prints the recorded Codex login route. A MISSING or STOP line from provisioning is a stop for that seat.
2. **Give the seat its identity, by the harness and route question 8 recorded**:
   - `pi` + `oauth`: `PI_CODING_AGENT_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" pi`, then `/login` inside the REPL, complete the browser flow, then `/exit`.
   - `pi` + `api_key`: write the seat's `auth.json` (`{"<provider>": {"type": "api_key", "key": "..."}}`, `0600`).
   - `pi` + `env`: export the provider env var in the shell that spawns the seat; do not write an `auth.json` stub because Pi checks `auth.json` before env.
   - `claude-code` + `oauth`: `CLAUDE_CONFIG_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" claude auth login --claudeai` (browser flow). This is the default Anthropic-subscription path.
   - `claude-code` + `api_key`: `CLAUDE_CONFIG_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" claude auth login --console` only when the principal explicitly chose the metered API-key route.
   - `codex` + `oauth`: `CODEX_HOME="$HOME/.pi-seats-<namespace>/<seat-name>" codex login` (or `codex login --device-auth` for headless).
   - `codex` + `api_key`: `printenv OPENAI_API_KEY | CODEX_HOME="$HOME/.pi-seats-<namespace>/<seat-name>" codex login --with-api-key`.
   Never paste a key into the conversation, the roster, or a bead. No `env` route exists for `claude-code` or `codex` seats; their drivers strip provider env vars from child processes so a subscription seat cannot silently bill a shell key.
3. **Verify the harness login before the binding probe**:
   - `pi`: `PI_CODING_AGENT_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" pi auth check --provider <provider> --json` (or the current Pi equivalent when the CLI names it differently). Semantics: `pi auth check --provider <provider> --json     # {"status":"ready",...} exit 0 | {"status":"not_ready",...} exit 1`.
   - `claude-code`: `CLAUDE_CONFIG_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" claude auth status` must report `loggedIn true`.
   - `codex`: `CODEX_HOME="$HOME/.pi-seats-<namespace>/<seat-name>" codex login status` must report the chosen route (for ChatGPT OAuth, `Logged in using ChatGPT`).
4. **Run the per-harness binding probe** — the check that the seat's account, provider and model actually resolve TOGETHER:
   - `pi`: `PI_CODING_AGENT_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" pi -p --no-session --provider <provider> --model <model> "Reply with exactly the word OK. Use no tools." </dev/null`
   - `claude-code`: `CLAUDE_CONFIG_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" claude -p --output-format json --model <model> --setting-sources project "Reply with exactly the word OK. Use no tools." </dev/null`
   - `codex`: `CODEX_HOME="$HOME/.pi-seats-<namespace>/<seat-name>" codex exec --json --skip-git-repo-check -s read-only -c approval_policy=never -m <model> "Reply with exactly the word OK. Use no tools." </dev/null`

   Expect `OK` from the binding probe. A probe that errors names the weakest link — dead login, missing key or env var, wrong model for the plan, revoked key — and the seat is not provisioned until it answers. For a `pi` seat, both the auth check and the binding probe select the account through `PI_CODING_AGENT_DIR` plus the credential route recorded for the seat — measured: the same probe answers `OK` under a logged-in/file-key directory, answers `OK` under an env-route directory when the provider env var is exported, and says "No API key found" only when no usable credential route is present. `--no-session` keeps the Pi probe out of the seat's session history; the provider and model are the ROSTER's, spelled out, because the probe exists to test exactly that pair (a bare `--provider` falls back to the default provider silently — the l0e gotcha again, and here it would make the probe vacuous). If question 8 marked a pin provisional, the selected harness's listing/resolution check is mandatory here before the binding probe, and any correction updates both `seats/seats.json` and the roster table in `wheelhouse/fleet/SEATS.md`. Do not proceed to step 6's smoke loop claiming a seat this probe has not passed.
...
- Fill `wheelhouse/crew/VERIFIER.md`'s `## This project` section from a distinct consumer-surface interview moment. Ask the principal to name the surfaces a stranger can actually walk for this install's claims. For a template-style fleet, offer these defaults for confirmation: README front door on the template's merge branch, first morning / generated start prompt, and upgrade runbook from a real consumer baseline. For a product fleet, ask for the app's staging URL, emulator or launch command, CLI command, API endpoint, or other public entry point. At least one surface must be named, or the section records the honest gap: `no consumer surface yet — walks impossible, claims close not-walked`. Do not leave the project half blank; a blank verifier section makes the walk gate look configured when it is not.
- Fill `wheelhouse/fleet/SEATS.md`'s `## This project` section from the answers to questions 7 and 8, **both parts**, quoting the two machine records step 3 wrote — `namespace=` read back out of `wheelhouse/.template-source`, and `seats/seats.json` — rather than your memory of the conversation. **The roster first, under a `### Roster` heading**: an intro line naming the seat root the recorded namespace decides (`$HOME/.pi-seats-<namespace>/` and where it is recorded), then one line per seat that was taken, mirroring `seats/seats.json` column for column — name, role brief, harness, provider, model, auth route, agent directory (`account.dir`) — with the verifier row saying it is the consumer-surface walker when the project took one, and with the commander's line carrying its "the principal's own session, external in the roster" shape; `generated/SEATS.md.example` in the template shows the table. `seats.json` is the machine record and decides; this table is the human record and must agree with it. Then `### Declined seats`: one line per seat that was refused, carrying the reason in the principal's words and the date. A roster listing only what was taken is indistinguishable from one where the question was never asked, which is the reason the interview walks the seats individually in the first place. If every seat was declined, the roster holds the commander line alone and the declined section holds the rest — that is a filled section, not an empty one. Add `### Walk cadence` as a pointer to the authoritative `walk_cadence_merges` value in `wheelhouse/INTEGRATOR.md`'s project half; do not duplicate the integer as a second source of truth. Its `## Contract` section — which carries the seat-accounting rule — stays verbatim like every other contract.
- Write `CLAUDE.md` at the root: commander-session context, product changes happen in product repos via workers, principal direction lands in `wheelhouse/ISA.md`'s Decisions before (or in the same breath as) it becomes described work, no dispatch happens without reading the ISA Goal, merges move Claims per `wheelhouse/INTEGRATOR.md`, new work the principal mentions becomes a described, prioritized task immediately, deadline tasks outrank everything, a tooling defect gets its own `Report <tool> issue:` task labelled `template-report` rather than a silent workaround, plus an `## Autonomy mandate` section carrying the principal directive from question 4b, the reserved actions from Q4, the merge policy from Q5, the rule that only reserved actions and genuine product-intent forks interrupt the principal, and the restart checklist: read ISA, read the graph, resume seats, pick up every in-flight branch or PR before starting new work. Add `generated/CLAUDE.md.example`'s `## Commander voice` section too: shipped commander prose addresses humans in tasks, intents, and outcomes; it does not say `bead`, does not use bare namespace ids as sentence subjects, and does not use mannered filler such as `kindly`, `please advise`, `circle back`, or `do the needful` except inside literal commands, paths, branch names, or quoted historical records with a human explanation beside them. The label is the one item on that list a program acts on later — `wheelhouse/GRAPH.md` says what it marks and who harvests it — so write it as the literal string, not as a description of one. `generated/CLAUDE.md.example`'s "Standing behavior" section has the `seats/fleet-gate.sh` bullet — carry it forward, worded for this project.
...
- Write `wheelhouse/STARTUP.md`: this project's cold-start card. It is what step 6 and the commander point at when it is time to actually start a session, so its job is to resolve rather than to restate — the seat MECHANICS live in `wheelhouse/fleet/SEATS.md`'s "Running a seat" paragraph and in `seats/README.md`, identically in every project, and this file is where those mechanics meet this project's real namespace and real seat names. Make it at least as complete as `generated/STARTUP.md.example`: a title and one-paragraph durability note, the minimum viable fleet if this project has one, the four operating sections below, and the shutdown section. None are optional:
  ...
  2. **Provisioning and spawning, from the roster you have just written.** One `seats/seat-env.sh <namespace> <seat-name>` line per roster seat — run once, safe to re-run — with a sentence saying it prints the one-time login flow for the harness and account that seat should BE (`PI_CODING_AGENT_DIR=... pi`, `CLAUDE_CONFIG_DIR=... claude auth login --claudeai`, or `CODEX_HOME=... codex login` / key login, matching question 8), and that the login-per-account rule is `wheelhouse/fleet/SEATS.md`'s seat-accounting section. Then the spawn lines: `bun seats/adapter.ts spawn <seat>` per seat, plus `status`, and a sentence saying `resume` is the warm reattach after a stop or reboot. Take the namespace, harnesses and names from the sections you have just written, not from your memory of the interview. There is nothing to paste into a seat — say so, because the role brief is injected at spawn. **Copy no seat name that is not on the roster**, and if the roster took no seats, say that in place of this section rather than inventing a specimen seat.
  3. **Watching the fleet.** How dispatches go out (`bun seats/adapter.ts dispatch <seat> <bead-id> ...`) and where each seat's events land (`seats/logs/<seat>.jsonl`), with `bun seats/adapter.ts status` as the always-available liveness view. Then the bridge, by name, because it is real machinery this install just copied and a vague pointer here is the difference between a cockpit that gets used and one that gets rediscovered months in: `seats/cockpit.sh <namespace>` builds (or re-attaches to — it never duplicates, and a re-run respawns a missing floor pane) the project's tmux session `wh-<namespace>`, commander pane on the left, starts `commander-inbox-poll.sh` inside that pane as the wrapper-independent inbox wake, and `bun seats/floor.ts` on the right is the read-only floor — spotlight one seat, rail of every seat's attention cues. Write the real namespace into that line. Point at `seats/README.md`'s "The bridge" section for the keys and what each cue means rather than restating them.
  4. **How work reaches the fleet.** The principal speaks backlog items to the commander, which files them as described tasks in the graph; workers report done on the task record with comments and the `needs-review` label without closing; reviewers gate branches with bench evidence; the commander/integrator closes only after merge per this project's merge policy. Use commander voice here too: say what the human task, intent, or outcome is, not just the internal graph id.
...
- **Check the pruning tool arrived with the seats machinery.** It is copied in step 2, and it replaces the old one-off worktree pruning runbook.

  ```bash
  test -x seats/prune.ts
...
Before you stage, make the worktree directory invisible to this repository. Workers create worktrees at `<root>/.wheelhouse-worktrees/<bead-id>` in both supported shapes because that is the single path the adapter, prune tool, and seat trust model agree on. That directory is runtime state: it is not part of the product, and in the single-repo shape it must not alter the product's committed ignore policy. Use `.git/info/exclude`, which is local to this checkout.

```bash
mkdir -p .git/info
if [ -s .git/info/exclude ] && [ -n "$(tail -c1 .git/info/exclude)" ]; then printf '\n' >> .git/info/exclude; fi
grep -qxF '.wheelhouse-worktrees/' .git/info/exclude 2>/dev/null \
  || printf '.wheelhouse-worktrees/\n' >> .git/info/exclude
```

The first line inside the block is not decoration. `>>` appends at the byte the file ends on, and an exclude file whose last line has no trailing newline — ordinary output from plenty of editors and generators — turns `node_modules/` and the new entry into the single line `node_modules/.wheelhouse-worktrees/`, which matches neither. Measured: the previously-ignored directory stops being ignored, the new one never starts, and the corrupted local exclude leaves every worktree visible as untracked dirt. Verified across all three starting states — no file, trailing newline, no trailing newline — three runs each and idempotent. It is an `if` rather than an `&&` chain because a chain whose guard is false exits non-zero, and here that is the healthy case: measured under bash and zsh, the chain form exits 1 when there is no file and 1 when the file already ends in a newline, reaching 0 only on the corrupted state it exists to repair — so the common path ends on a non-zero status and any caller, CI step or wrapper that reads it calls the install failed. It does NOT abort a `set -e` script mid-run — `A && B` is an AND-OR list, which `set -e` exempts, and execution continues past the false guard under bash, zsh and sh alike. The `if` runs the same command on the same condition and exits 0 either way.

Measured: before, `git status --short` at the install root reports `?? .wheelhouse-worktrees/`; after, it reports nothing for it. If this checkout sits inside some other repository, that outer repository is not this install's to edit; say so to the principal rather than leaving dirt they will find later and attribute to their own working copy.
...
Step 5's checks verified the FILES. This step verifies the FLEET, by the only measure that means anything: one bead travels the whole loop — dispatched through the adapter to a real seat, worked, reported with evidence, and judged by the reviewer — and the verdict lands where the graph can read it. Until that has happened once, "the install works" is a claim about bytes on disk.
...
1. **Spawn what the smoke needs**, per `wheelhouse/STARTUP.md`: the worker seat (`bun seats/adapter.ts spawn <worker-seat>`), and confirm with `bun seats/adapter.ts status`. The reviewer used for `seats/verify.ts` needs no spawn — the dispatcher starts its one-shot verdict process at the moment of judgment. The verifier/walker also needs no spawn; `seats/walk.ts` starts it when a claim walk is due.
...
4. **Verdict, by the one-shot reviewer**: `bun seats/verify.ts <bead-id> fleet/<bead-id> <worker-seat>`. The exit code IS the verdict — 0 APPROVE, 2 BOUNCE, 3 DISCOVER, 1 no-verdict-exists — and the full output lands in `seats/verdicts/<bead-id>.md`, which git ignores because it is a working copy. **Transcribe the decisive extract onto the bead before citing it** (`wheelhouse/GRAPH.md`, "Where evidence lives"), then close the smoke bead naming the verdict. If the roster has no reviewer seat, or `verify.ts` refuses because the only reviewer shares the author's account, the verdict comes from the solo path instead — the principal, or a fresh session that authored nothing — recorded on the bead in the same shape, and say plainly that the mechanical reviewer leg went unexercised. Before the install's first ISA claim closes, run the first claim walk with `bun seats/walk.ts <claim-ref> --surface <kind>:<spec> --out evidence/<bead-id>/<surface-walk> --verifier <verifier-seat>` and cite the walk verdict plus transcript in the claim, or state `Not walked:` explicitly per `INTENT.md`.
