...
8. **Seat harness, provider, auth route and model — a round of its own, run after the crew is chosen, walked over the seats question 7 ACTUALLY took.** Never one blanket question. A combined harness/provider/model question fails the same two ways the old provider+model question failed: its options cannot reflect a mixed crew, and its model options come from the interviewer's memory instead of the selected harness's account. Walk each taken seat by name, one at a time. Every seat gets its own explicit answer: "same as the previous seat" is an answer the principal gives, not a default you assume.
   ...
   Ask Q8 as four separate **choice rounds** after Q7 has chosen the roster. A round may contain one question per taken seat in a single `AskUserQuestion` call, but a round never mixes question kinds. This keeps the turn count bounded while preserving the narrowing order.
   ...
   **Model — pinned from the selected harness's own live source, taken at the moment of asking.** For `pi`, run `PI_CODING_AGENT_DIR="$HOME/.pi-seats-<namespace>/<seat-name>" pi --list-models | awk '$1=="<provider>"'` and offer only ids it prints for that provider. For `claude-code`, there is no list command in `claude --help`; offer only documented full model ids you can verify for that account, or type the documented alias as a correction path and then record the full id the binding probe's `system/init` reports. For `codex`, use the driver/app-server model list (`bun seats/adapter.ts models <seat>` once that driver ships, or the one-line app-server `model/list` probe named in `wheelhouse/research/harness-contract.md`) and never a remembered catalog. A model the selected harness/account cannot list or resolve is a typo the seat discovers at first dispatch.
