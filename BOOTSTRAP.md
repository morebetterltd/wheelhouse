...
8. **Seat models — a round of its own, run after the crew is chosen, walked over the seats question 7 ACTUALLY took.** Never one blanket question. A combined "provider + pinned model" question fails two ways, both observed on a real install (2026-09-01): its options cannot reflect a mixed crew — that install chose two workers, a reviewer, a verifier and a designer, was offered one-model-for-every-seat, and had to spell the per-seat choices into a free-text notes field no record reads again — and its model options come from the interviewer's own pretraining, which ages the moment it is written. So this round walks each taken seat by name, one at a time, and every seat gets its own explicit answer: "same as the previous seat" is an answer the principal gives, not a default you assume. With `AskUserQuestion`, run one round per seat, and build the MODEL options from the live listing below at the moment of asking, never from this file and never from memory. Field precedent: a real five-seat roster in this post-bi9 shape fits as one question 7 crew round plus one question 8 round per taken seat; do not collapse those per-seat question 8 rounds back into a blanket question. When `AskUserQuestion` is available, every option label must be a selectable value the installer can write directly or map without prose: count labels like `2 workers`, take/decline labels like `Take verifier`, provider labels like `openai-codex`, auth labels like `oauth` or `api_key`, and model labels that are exactly the live model ids. Do not offer an `Other`, `custom`, `notes`, `same as previous`, or free-text label for ordinary choices; free prose is only the fallback when the tool is absent or when the principal explicitly chooses the correction path you named. Keep headers literal and seat-scoped — `Question 8 — worker-2 model`, `Question 8 — verifier provider` — and do not put `name`, `same as`, or another seat's role in a provider/model prompt. For model choice, show the live listing in the question text if useful, but make the option labels the exact ids it printed (`gpt-5.4`, `claude-sonnet-4-5`), not explanatory phrases.

   Three answers per seat, in this order, because each narrows the next. Keep the three question kinds grammatically distinct so the answer record stays machine-readable: provider questions literally ask "Which provider?" and offer provider labels (`openai-codex`, `anthropic`, `openai`); auth questions literally ask "Which auth route?" and offer auth labels (`oauth`, `api_key`, `env`); model questions literally ask "Which model?" and offer model labels from that one provider's live listing. Do not ask "whose model?" for provider, and do not mix ids from multiple providers in one model question.

   Known-good `AskUserQuestion` shapes for the standard mixed scratch roster are intentionally boring: Q7 labels `2 workers`, `1 reviewer`, `Take verifier`, `Take designer`; Q8 provider labels `openai-codex` for each worker, `anthropic` for reviewer/verifier when OpenRouter is not available, `openai` for designer; Q8 auth labels `oauth` for `openai-codex` and `api_key`/`env` for API-key providers; Q8 model labels are exact ids from the filtered listing, for example `gpt-5.4`, `gpt-5.5`, `claude-sonnet-4-5`, `claude-opus-4-5`. That list is not a model catalog — the live listing still decides — it is the label shape that prevents free-text answers.
