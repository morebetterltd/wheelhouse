...
7. **Seats — derive the namespace once, then walk the roster one seat at a time.** Do not offer a fleet shape and ask whether they want it. A roster proposed whole is accepted whole, and the seat that goes missing is the one nobody was asked about: today that is reliably the designer, because its absence looks like a smaller install rather than a gap.
   ...
   **Derive it once and record it once.** The recorded value goes in `wheelhouse/fleet/SEATS.md`'s `### Seat namespace` section and nowhere else; the roster's names, `wheelhouse/STARTUP.md`'s launch blocks and its `wire-seats.sh` invocation are all derived from it at the moment you write them. Ask a second time and you will get a second answer, and the surface that disagrees will be the one nobody reads until a seat cannot be reached.
...
- Fill `wheelhouse/fleet/SEATS.md`'s `## This project` section from question 7's answers, **all three parts**. `### Seat namespace` first, because the other two are derived from it: the namespace string and the seat root it names, `$HOME/.claude-seats-<namespace>`. This is the ONE place that value is recorded, and every other seat-naming surface you are about to write — the roster's names, `wheelhouse/STARTUP.md`'s `SEATS_ROOT` exports and `CLAUDE_CONFIG_DIR` lines, its `wire-seats.sh` invocation — is written FROM it rather than alongside it. Write the section before the roster and read it back when you write STARTUP.md; a second recollection of a value you already recorded is how the two come to disagree. Then **both halves of the roster**: one roster line per seat that was taken, carrying its namespaced name, and one line under `### Declined seats` per seat that was refused, carrying the reason in the principal's words and the date. A roster listing only what was taken is indistinguishable from one where the question was never asked, which is the reason the interview walks the seats individually in the first place. If every seat was declined, the roster holds the commander line alone and the declined section holds the rest — that is a filled section, not an empty one. Its `## Contract` section — which carries the seat-accounting rule — is copied verbatim like every other contract.
...
- Write `wheelhouse/STARTUP.md`: this project's cold-start card. It is what step 8 and the commander point at when it is time to actually start a session, so its job is to resolve rather than to restate — the launch MECHANICS live in `wheelhouse/fleet/SEATS.md`'s "Launching a seat" section, identically in every project, and this file is where those mechanics meet this project's real root path and real seat names. Four things, in this order, and none of them optional:
  ...
  2. **One block per seat on the roster**, using that seat's real name, this project's real seat root and this project's real project root — the `export SEATS_ROOT=...` / `export CLAUDE_CONFIG_DIR=...` / `cd` / `claude` lines from the launch procedure with the placeholders already substituted, so the principal can paste rather than translate. Take the seat root and the names from `### Seat namespace` and `### Roster` in the file you have just written, not from your memory of the interview. Then point at `wheelhouse/fleet/SEATS.md` by name for what those lines mean, the login-per-account rule, the standing prompt to paste, and what the seat does next. **Copy no seat name that is not on the roster**, and if the roster took no seats, say that in place of this section rather than inventing a specimen seat.
  3. **Discovery, then the roll call.** `wheelhouse/runbooks/wire-seats.sh` — with this project's real root in the `cd` and its `SEATS_ROOT` exported, both from the file you just wrote — run by the principal after launching or relaunching any seat, then a commander roll call before it dispatches anything, then each seat messaging the commander unprompted to prove the reverse leg. Say that `SEATS_ROOT` is not decoration: without it the script enumerates the shared default root, and on a machine hosting a second wheelhouse that wires the other project's seats into this commander's registry. Point at `wheelhouse/runbooks/SEAT_DISCOVERY.md` for what those three steps mean, why a round trip is not the check, and what the roll call asserts about ambiguous names. An unwired seat looks exactly like an idle one, and an un-rolled fleet is reachable but anonymous.
