...
8. **Seat harness, provider, auth route and model — a round of its own, run after the crew is chosen, walked over the seats question 7 ACTUALLY took.** Never one blanket question. A combined harness/provider/model question fails the same two ways the old provider+model question failed: its options cannot reflect a mixed crew, and its model options come from the interviewer's memory instead of the selected harness's account. Walk each taken seat by name, one at a time. Every seat gets its own explicit answer: "same as the previous seat" is an answer the principal gives, not a default you assume.
   ...
   Four answers per seat, in this order, because each narrows the next. Keep the question kinds grammatically distinct so the answer record stays machine-readable: harness questions literally ask "Which harness?" and offer labels exactly `pi`, `claude-code`, `codex`; provider questions literally ask "Which provider?" and offer only providers valid for the harness just chosen for that seat; auth questions literally ask "Which auth route?" and offer only route labels exactly from `oauth`, `api_key`, `env`, restricted to the routes valid for that harness/provider pair; model questions literally ask "Which model?" and offer only model ids from that harness/account's live listing or binding probe. Do not batch a later question until the narrowing answer before it is known: a model question that includes every harness's options is a failed Q8, not a helpful shortcut. Do not combine auth and model into one question or one option label: `api_key` is an auth option and `gpt-5.5` is a model option, while `api_key + gpt-5.5`, `auth_json + ...`, and `env_var + ...` are invalid free text. Do not offer an `Other`, `custom`, `notes`, `same as previous`, or free-text label for ordinary choices; free prose is only the fallback when the tool is absent or when the principal explicitly chooses a correction path you named.

   Known-good `AskUserQuestion` shapes for the standard mixed scratch roster are intentionally boring: Q7 labels `2 workers`, `1 reviewer`, `Take verifier`, `Take designer`; Q8 harness labels `pi`, `claude-code`, `codex`; the epic's falsifier roster is mixed — worker on Claude Code, reviewer on Codex, verifier on Pi — so the interview must not assume one harness for the fleet. Provider labels narrow by harness: `pi` uses today's live `pi --list-models` providers; `claude-code` uses `anthropic` only, because that harness is first-party Anthropic; `codex` uses `openai-codex` for a ChatGPT subscription or `openai` for an API key. Auth labels narrow by harness: `pi` offers `oauth`, `api_key`, `env` as today; `claude-code` defaults to `oauth` through `claude auth login --claudeai` and may offer `api_key` through `claude auth login --console` as metered/non-default, but never `env`; `codex` offers `oauth` (`codex login` or `codex login --device-auth`) or `api_key` (`printenv OPENAI_API_KEY | codex login --with-api-key`), but never `env`. Model labels are just model ids for the selected provider/harness — never rows combining harness, provider, auth, and model.
