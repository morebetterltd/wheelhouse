...
- Cockpit keeps the bridge window to the commander/floor two-pane layout; subagent panes split into the bridge are moved into their own tmux windows automatically.
...
- **The fleet's own one-shot reviewer — when its precondition holds.** The template's review gate is already an ephemeral mind: `seats/verify.ts` dispatches a one-shot reviewer pass on a finished branch and maps its verdict to an exit code (`seats/README.md`, "Verifying a branch"). Its precondition is the one thing a solo install may not have: the review account must be distinct from the author's, and `verify.ts` enforces that on disk — same agent directory is a loud STOP, not a warning — so a change authored on the only account you hold cannot be verified on that account. When the precondition holds, this is the preferred shape, because the independence is enforced rather than practiced; when it does not, the two minds below remain.
...
The walk command exits `0` for `WALKED-DONE`, `2` for `WALKED-NOT-DONE`, `3` for `COULD-NOT-WALK`, `4` for missing/ambiguous/malformed verdict output, and `5` for preflight or credential refusal. The default walk budget is 1800000 ms (30 minutes); for a single expensive walk, run with `WHEELHOUSE_WALK_TIMEOUT_MS=<ms>`. A timed-out walk keeps the partial transcript and names the phase it was in. Rule of thumb for GUI surfaces: a walk and a bench never share the host display; put the walk on a second display or headless path, or serialize them from the commander. A closing claim cites the walk verdict and transcript evidence home, or it carries the explicit not-walked statement `INTENT.md` permits; silence is the defect. Treat `WALKED-NOT-DONE` like a field report: file the failing step as a bead with a Trace line to the walk, then decide whether the claim stays pending, narrows, or closes not-walked.
