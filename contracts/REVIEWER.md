...
- When this install has a host build budget (`seats/host-budget.json` exists and the one-shot verifier PATH includes `seats/bin`), every build and test you run goes through the shim; never call the raw tool by absolute path, by a PATH that skips `seats/bin`, or by any other escape hatch. Build caches are shared under the host lock by default. Per-worktree cache isolation is allowed only with a written reason on the bead, because isolation multiplies disk and compile load.
