...
The default reviewer is EPHEMERAL — a session dispatched for one bead and discarded once the verdict lands (`wheelhouse/fleet/SEATS.md`, Lifecycle). Such a seat exists only while it is taking a turn. Work put into the background is therefore put past the end of the seat, and the seat is not there to collect it. Measured on a fleet whose bench builds an iOS app: reviewer seats started a ten-minute build, went idle, and did not reliably wake when it exited — verdicts arrived hours late, or only after the commander probed. Worker seats running the same build finished normally, because a worker still had work in hand while it ran and so was still inside its turn when it ended. The variable is not the bench and not the role. It is whether anything was left to do while the clock ran.
...
- **Establish that a backgrounded bench survives its seat at all.** Whether a process started by a session keeps running after that session ends is a fact about your harness, and it varies by harness and by configuration. Measure it before building a review on it. If it does not survive, this shape is unavailable to you, and what remains is the first answer or a party that outlives the seat running the bench itself.
