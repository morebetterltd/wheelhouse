...
- When closing a bead after review and merge, prune that closed bead's scratch before reporting the close: run `bun seats/prune.ts scan > <reviewed-scan>` from the install root, review rows for that bead, then run `bun seats/prune.ts prune --from-file <reviewed-scan> --yes --categories bead-runs,bead-tmp,bead-simulator,xctest-devices` for the safe closed-bead rows. The tool's safety rule is closed-bead-only for bead-named scratch: open or in-progress bead scratch is emitted as `needs-review` and is never acted on. `xctest-devices` is safe only when its simulator set is idle and no `xcodebuild` process is running.
