...
A committed capture must also be scrubbed before it is committed: no local usernames, home directories, machine-identifying hostnames, or tmp paths — replace them with neutral placeholders (`[user]`, `[tmpdir]`) at capture time. This repo is public-destined, and an `ls -l` owner column or a `/var/folders/...` path in evidence outlives every intention to clean it later.
