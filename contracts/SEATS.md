...
A persistent seat process the commander owns: one long-lived harness per seat, started and driven by the adapter (`seats/adapter.ts`), addressable by its roster name. The roster (`seats/seats.json`) says what the seat IS — role, optional `harness` (`pi` by default; `claude-code` and `codex` supported), provider/model where that harness needs them, and the account directory that holds its identity (`account.dir`). It optionally also carries `account.authRoute` — which route (`oauth`, `api_key`, `env`, or the Claude-only smoke-test escape `default`) gave the seat its identity — `shadow`, a boolean marker for a mirror seat whose output is recorded but never gates or carries assigned work, and `skills`, an array of per-seat skill directories passed to Pi as `--skill` after tilde expansion and rejected when a path is missing; these are durable and safe to commit because they are route/behavior/path names, never credentials, and a roster written before any optional field existed simply omits it. `account.authRoute=default` is only for Claude Code evidence/smoke runs on the operator's already-logged-in default Claude Code account: it launches with `CLAUDE_CONFIG_DIR` unset and must not be committed for a rostered production seat. Claude Code seats launch in measured-safe unattended mode (`--permission-mode acceptEdits --permission-prompts none` plus the roster `allowedTools` list); a project section may opt a seat up to a broader mode, but this fleet did not probe that broader mode and the template default does not ship it. The runtime record (`seats/state.json`) says what currently embodies it — pid, session, log, FIFO — written by the adapter, never by hand. The role is injected at spawn: the adapter appends the role's crew brief to the harness system prompt. A running seat has its role already; nothing is pasted into it, and a seat behaving as if it had no brief was spawned wrong, not briefed wrong. A `researcher` role is a knowledge-only crew role for decision-grade sourced reports, not product edits or graph work beyond its own report comment.
...
Two commands use a rostered identity without creating or resuming a persistent seat process. `seats/verify.ts` is a one-shot REVIEWER pass for a bead and branch: it uses the verifier seat's account directory through that seat's harness driver (`PI_CODING_AGENT_DIR`, `CLAUDE_CONFIG_DIR`, or `CODEX_HOME`), injects `REVIEWER.md`, writes a verdict, and exits with no warm session. `seats/walk.ts` is a one-shot VERIFIER walk on a consumer surface: it uses the verifier identity through that seat's harness driver, injects `VERIFIER.md`, prompts only with the claim and named surface, writes the walk verdict, and exits with no bead-review context.
...
Provision once, then run with the adapter; the commands and what each one does live in `seats/README.md`, and this paragraph is deliberately all this contract says about launching. Provisioning is `seats/seat-env.sh <namespace> <seat-name>` — it creates the seat's agent directory, pre-grants trust for the project root, and prints the one-time credential flow for the account that seat should BE: `PI_CODING_AGENT_DIR=... pi`, `CLAUDE_CONFIG_DIR=... claude auth login --claudeai`, or `CODEX_HOME=... codex login` / key login, matching the roster. From then on the seat is `bun seats/adapter.ts spawn <seat>` to start, `dispatch` to hand it a bead, `steer` to redirect mid-turn, `status` for liveness, `stop` and `resume` for the graceful stop and the warm reattach. The one-shot bead review is `bun seats/verify.ts <bead-id> <branch> <author-seat>` and injects the REVIEWER brief; the one-shot consumer walk is `bun seats/walk.ts <claim-ref> --surface <kind>:<spec>` and injects the VERIFIER brief. Pi and Claude Code one-shots pass the brief with `--append-system-prompt`; Codex one-shots use `codex exec -c developer_instructions=...` with `--skip-git-repo-check -s read-only -c approval_policy=never` (measured on codex-cli 0.144.0). If any of those refuse to run, the refusal names the provisioning step that was skipped; the refusal is the guard working.
