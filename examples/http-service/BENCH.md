...
**6. It always tears down.** `trap cleanup EXIT INT TERM`, containers and network removed on every path. Three things learned by measuring rather than assuming, all encoded in the script — the first corrected after a stranger's read disproved its mechanism, the third only after a reviewer found it in the first version of this file:
...
- Worse, when a signal does reach the handler, the trap can see `$? = 0` — the run was killed partway and nothing had failed yet — so an interrupted run exits 0 and reports success to anything reading the exit code. The script carries a `PASSED` flag set only on the success path, and the teardown forces a non-zero exit when it is unset. A bench that can exit 0 without finishing is not a bench.
