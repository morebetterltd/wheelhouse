...
function runCached(cmd: string[]): string {
  const key = cmd.join("\0"), now = Date.now(), hit = bdCache.get(key);
  if (hit && now - hit.at < BOARD_CACHE_MS) return hit.value;
  let out = "";
  try { const proc = Bun.spawnSync(cmd, { cwd: ROOT, stdout: "pipe", stderr: "pipe" }); out = proc.success ? new TextDecoder().decode(proc.stdout) : ""; } catch { out = ""; }
  bdCache.set(key, { at: now, value: out });
  return out;
}
...
function bdList(args: string[]): Issue[] { return parseIssueList(runCached(["bd", "list", "--json", "--limit", "0", ...args])); }
function readyIds(): Set<string> { return new Set(runCached(["bd", "ready"]).split(/\r?\n/).map(l=>l.trim().split(/\s+/)[0]).filter(Boolean)); }
function issueMap(): Map<string, Issue> { const m=new Map<string,Issue>(); for(const row of bdList([])) if(row?.id) m.set(row.id,row); return m; }
...
function buildBoard(): Board {
  const all=issueMap(), ready=readyIds(), openNeeds=openNeedByBead();
  ...
  for(const i of bdList(["--status","in_progress"])) if(i?.id && !isDependencyBlocked(i)) add(1, boardCard(i,"in-progress",seatFor(i.id,["worker"]) || i.assignee || "in progress",i.started_at || i.created_at));
  for(const i of bdList(["--label","needs-review"])) if(i?.id && !isDependencyBlocked(i)) add(2, boardCard(i,"in-review",verdictBounce(i.id) ? "sent back" : (seatFor(i.id,["reviewer","verifier"]) || "waiting for a reviewer"),i.started_at || i.created_at));
