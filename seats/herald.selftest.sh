...
# Builds a temporary project with synthetic seat logs and a fake tmux; never
...
json_count() {
  local expr="$1"
  node -e 'const fs=require("fs"); const [file,expr]=process.argv.slice(1); const rows=fs.existsSync(file)?fs.readFileSync(file,"utf8").trim().split(/\n/).filter(Boolean).map(JSON.parse):[]; console.log(rows.filter(Function("r", "return "+expr)).length)' "$PROJ/seats/inbox.jsonl" "$expr"
}
...
if [ $RC -eq 0 ] && echo "$OUT" | grep -q 'appended 3 wake event'; then pass "captures settle, distress, and sentinel events"
else fail "herald --once did not capture three events (rc=$RC): $OUT"; fi
...
if [ "$(line_count "$DRAIN1")" = 3 ] && [ ! -s "$DRAIN2" ] && [ "$(cat "$PROJ/seats/inbox.cursor")" = "$(wc -c < "$PROJ/seats/inbox.jsonl" | tr -d ' ')" ]; then
  pass "drain cursor emits unread wake events once, then advances to EOF"
else
  fail "drain cursor failed: drain1=$(line_count "$DRAIN1") drain2_bytes=$(wc -c < "$DRAIN2" | tr -d ' ') cursor=$(cat "$PROJ/seats/inbox.cursor" 2>/dev/null || echo missing)"
fi
...
state="${FAKE_TMUX_STATE:?}/session"
cmd="${1:-}"; shift || true
case "$cmd" in
  has-session) [ -f "$state" ]; exit $? ;;
  new-session) mkdir -p "$(dirname "$state")"; : > "$state"; exit 0 ;;
  list-windows) [ -f "$state" ] && echo bridge; exit 0 ;;
  list-panes) echo pane0; echo pane1; exit 0 ;;
...
FAKE_TMUX_STATE="$FIX/tmux" PATH="$FIX/bin:$PATH" WHEELHOUSE_TMUX_SOCKET=herald-test "$PROJ/seats/cockpit.sh" demo > "$FIX/cockpit1.out" 2>&1
RC=$?
...
sleep 0.4
FAKE_TMUX_STATE="$FIX/tmux" PATH="$FIX/bin:$PATH" WHEELHOUSE_TMUX_SOCKET=herald-test "$PROJ/seats/cockpit.sh" demo > "$FIX/cockpit2.out" 2>&1
RC=$?
