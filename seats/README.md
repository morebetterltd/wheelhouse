...
Codex harness notes: the driver runs a resident `codex app-server` shim under `CODEX_HOME=<account.dir>` (unless `authRoute=default` is explicitly used for evidence), strips provider API keys from the child environment, injects the role brief via `developerInstructions`, and normalizes app-server events to the same Pi-shaped seat log. Measured Codex issues: codex-cli 0.144.0 may print the expected-noise stale-model-cache line `ERROR codex_models_manager::cache: failed to load models cache: missing field \`base_instructions\`` to stderr when an old `models_cache.json` lacks the new field; this is harmless until fixed upstream, but it appears in the seat `.stderr.log`. An unauthenticated `CODEX_HOME` can also spend about 45 seconds retrying 401s before failing; provisioning/probe flows therefore login-check Codex credentials before real work.
