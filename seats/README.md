...
| `harness` | Optional harness driver, one of `pi`, `claude-code`, or `codex`; absent means `pi` for existing rosters. This release implements `pi` and `codex`; `claude-code` remains a sibling driver branch in review. If a local adapter command sees an unimplemented harness, it STOPs naming the seat instead of spawning Pi by accident. |
