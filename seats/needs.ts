...
function cmdSay(argv:string[]){ const {pos}=parseArgs(argv); const [id,...rest]=pos; if(!id||!rest.length) die("usage: needs.ts say <id> <text>",2); const text=rest.join(" "); humanTextGuard(text,"say text"); getNeed(id); appendEvent({type:"message",id,at:now(),from:"commander",via:"cli",text}); console.log(id); }
function cmdAnswer(argv:string[]){ const {pos,flags}=parseArgs(argv); const [id,...rest]=pos; if(!id||!rest.length) die("usage: needs.ts answer <id> <text> [--via desk|cli|<transport>]",2); const n=getNeed(id), text=rest.join(" "); const choice=n.opened.options.find(o=>o.label===text||o.text===text)?.label; appendEvent({type:"answered",id,at:now(),from:"human",via:one(flags,"via")??"cli",text,choice}); console.log(id); }
