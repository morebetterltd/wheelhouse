...
export type NeedEvent =
 ...
 | { type:"read"; id:string; at:string; by:string; via:string };
export interface Option { label:string; text:string }
export interface NeedFold { id:string; state:"open"|"answered"|"closed"; opened:Extract<NeedEvent,{type:"opened"}>; messages:Extract<NeedEvent,{type:"message"}>[]; answer?:Extract<NeedEvent,{type:"answered"}>; closed?:Extract<NeedEvent,{type:"closed"}>; read?:Extract<NeedEvent,{type:"read"}> }
...
export function lastHumanEventAt(n:NeedFold):string|undefined{ const times=[n.answer?.at,...n.messages.filter(m=>m.from==="human").map(m=>m.at)].filter(Boolean) as string[]; return times.sort().at(-1); }
export function isUnread(n:NeedFold):boolean{ if(n.state==="closed") return false; const last=lastHumanEventAt(n); return !!last && (!n.read || n.read.at < last); }
export function markRead(id:string, by="commander", via="cli"){ getNeed(id); appendEvent({type:"read",id,at:now(),by,via}); }
...
function cmdRead(argv:string[]){ const {pos,flags}=parseArgs(argv); const id=pos[0]; if(!id) die("usage: needs.ts read <id> [--by commander]",2); markRead(id, one(flags,"by")??"commander", "cli"); console.log(id); }
function cmdShow(argv:string[]){ const {pos}=parseArgs(argv); const id=pos[0]??""; markRead(id,"commander","show"); console.log(JSON.stringify(getNeed(id),null,2)); }
function cmdList(argv:string[]){ const {bools}=parseArgs(argv); let needs=Array.from(fold().values()); if(bools.has("unread")) needs=needs.filter(isUnread); else if(!bools.has("all")) needs=needs.filter(n=>n.state!=="closed"); needs.sort((a,b)=>(a.state==="open"?0:1)-(b.state==="open"?0:1)||b.opened.at.localeCompare(a.opened.at)); if(bools.has("json")) console.log(JSON.stringify(needs,null,2)); else for(const n of needs) console.log(`${n.id} ${n.state} ${n.opened.kind} — ${n.opened.title}${n.answer?` answer=${n.answer.text}`:""}`); }
if (import.meta.main) { const [cmd,...rest]=process.argv.slice(2); if(cmd==="open") cmdOpen(rest); else if(cmd==="say") cmdSay(rest); else if(cmd==="answer") cmdAnswer(rest); else if(cmd==="show") cmdShow(rest); else if(cmd==="read") cmdRead(rest); else if(cmd==="list") cmdList(rest); else if(cmd==="close") cmdClose(rest); else die("usage: needs.ts open|say|answer|show|read|list|close ...",2); }
